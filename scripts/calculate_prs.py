...
def match_snps_by_position(
    ...
    for record in vcf:
        chrom = normalize_chrom(record.chrom)
        key = (chrom, int(record.pos))  # Ensure pos is int for consistent matching
    ...
    for idx, (rsid, nucleus_snp) in enumerate(nucleus_items):
        ...
        # Convert GT tuple to string format (pysam returns tuple like (0, 1))
        if isinstance(gt, tuple):
            gt_str = '/'.join(str(a) if a is not None else '.' for a in gt)
        else:
            gt_str = str(gt)
        ...
        dosage, flipped = get_genotype_dosage(gt_str, ref, alt, nucleus_snp.a1)
        ...
        if dosage is not None:
            ...
            if dropped <= 5 and test_limit:
                print(f"  Debug dropped {rsid}: VCF={ref}->{alt}, A1={nucleus_snp.a1}, GT={gt}")
