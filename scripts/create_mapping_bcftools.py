...
def extract_mappings_from_dbsnp(dbsnp_vcf: str, rsid_list_file: str, output_tsv: str):
    ...
    Uses bcftools view with ID filter for efficient extraction.
    ...
    if not Path(index_file).exists():
        print("Indexing dbSNP VCF (this may take 5-10 minutes)...")
        print("This is a one-time operation - index will be reused next time")
        ...
        try:
            ...
            print("Continuing without index (will be slower)...")
    
    # Use bcftools view with ID filter - MUCH more efficient
    # This only extracts variants matching our rsIDs
    print("Extracting matching variants from dbSNP VCF...")
    print("(This may take 20-40 minutes, but uses efficient filtering)")
    
    # Load rsIDs we're looking for
    rsid_set = set()
    ...
    # Use bcftools view to filter by ID, then query to extract fields
    # Step 1: Filter VCF to only matching IDs
    print("Filtering dbSNP VCF for matching rsIDs...")
    filter_cmd = [
        'bcftools', 'view',
        '-i', f'ID=@{rsid_list_file}',
        '-O', 'v',  # Output as VCF
        dbsnp_vcf
    ]
    
    # Step 2: Query filtered results
    query_cmd = [
        'bcftools', 'query',
    ...
    try:
        # Pipe filter output to query
        print("Processing variants (showing progress every 10k matches)...")
        matches = 0
        ...
        filter_process.stdout.close()  # Allow filter_process to receive SIGPIPE
        
        with open(output_tsv, 'w') as out_f:
            out_f.write("rsID\tchr\tpos\tref\talt\n")
                ...
                if len(parts) >= 5:
                    ...
                    for rsid in rsids_in_field:
                        if rsid in rsid_set:
                            ...
                            ref = parts[3]
                            alt = parts[4]
                            out_f.write(f"{rsid}\t{chrom}\t{pos}\t{ref}\t{alt}\n")
                            matches += 1
                            rsid_set.discard(rsid)
                            if matches % 10000 == 0:
                                print(f"  Found {matches:,} matches so far...")
                            break
        ...
        if filter_process.returncode != 0:
            stderr = filter_process.stderr.read().decode()
            if stderr and 'not found' not in stderr.lower():
                print(f"Warning from bcftools view: {stderr}", file=sys.stderr)
        
        print(f"\nExtracted {matches:,} mappings saved to {output_tsv}")
        if len(rsid_set) > 0:
            print(f"Note: {len(rsid_set):,} rsIDs not found in dbSNP VCF (this is normal)")
        return True
